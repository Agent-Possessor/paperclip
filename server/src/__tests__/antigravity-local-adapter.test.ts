...
describe("antigravity_local parser", () => {
  it("extracts session, summary, usage, cost, and terminal error message from v0.38 stream-json output", () => {
    ...
    expect(parsed.sessionId).toBe("antigravity-session-1");
...
describe("antigravity_local stale session detection", () => {
  it("treats missing session messages as an unknown session error", () => {
    expect(isAntigravitySessionUnrecoverableError("", "unknown session id abc")).toBe(true);
    expect(isAntigravitySessionUnrecoverableError("", "checkpoint latest not found")).toBe(true);
  });
});

describe("antigravity_local turn-limit detection", () => {
  it("detects structured turn-limit signals and exit code 53", () => {
    expect(isAntigravityTurnLimitResult({ status: "turn_limit" })).toBe(true);
    expect(isAntigravityTurnLimitResult({ stopReason: "max_turns_exhausted" })).toBe(true);
    expect(isAntigravityTurnLimitResult(null, 53)).toBe(true);
  });
  ...
  it("does not detect turn-limit exhaustion from unstructured error text", () => {
    expect(isAntigravityTurnLimitResult({ error: "max_turns reached" })).toBe(false);
  });
});
...
describe("antigravity_local cli formatter", () => {
  it("prints init, v0.38 assistant, result, and error events", () => {
    ...
    expect(joined).toContain("Antigravity init");
