...
async function writeFakeAntigravityCommand(commandPath: string): Promise<void> {
...
async function writeFailingAntigravityCommand(
  commandPath: string,
  options: {
...
describe("antigravity execute", () => {
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-antigravity-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agy");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes turn-limit exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-antigravity-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agy");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingAntigravityCommand(commandPath, {
      stdoutLines: [
        {
          type: "result",
          subtype: "error",
  ...
  it("normalizes Antigravity exit code 53 as max-turn exhaustion", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-antigravity-exit-53-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agy");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingAntigravityCommand(commandPath, {
      stderr: "Antigravity stopped because the max turns limit was reached.",
      exitCode: 53,
    });
  ...
  it("does not normalize unstructured turn-limit text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-antigravity-max-turn-text-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agy");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingAntigravityCommand(commandPath, {
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-antigravity-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agy");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.argv).toContain("antigravity-session-1");
