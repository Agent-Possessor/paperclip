...
export async function testEnvironment(
  ...
  const runId = `kiro-envtest-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  ...
  try {
    ...
    checks.push({
      code: "kiro_install_probe_failed",
      level: "warn",
  ...
  if (!commandLooksLike(command, "kiro-cli")) {
    checks.push({
      code: "kiro_custom_command",
      level: "info",
      message: "Using a custom Kiro command path.",
      detail: command,
    });
  }
