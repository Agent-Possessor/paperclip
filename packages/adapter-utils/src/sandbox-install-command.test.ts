...
describe("buildSandboxNpmInstallCommand", () => {
  it("installs globally as root, via sudo when available, and under ~/.local otherwise", () => {
    const command = buildSandboxNpmInstallCommand("mock-package");
    expect(command).toContain("if [ \"$(id -u)\" -eq 0 ]; then npm install -g 'mock-package';");
    expect(command).toContain("sudo -E npm install -g 'mock-package'");
    expect(command).toContain("npm install -g --prefix \"$HOME/.local\" 'mock-package'");
  });
